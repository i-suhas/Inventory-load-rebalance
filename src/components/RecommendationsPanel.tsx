...
const ACTION_STYLE: Record<string, { border: string; icon: string; badge: string }> = {
  "Transfer Inventory": { border: "border-blue-500/30", icon: "🔀", badge: "bg-blue-500/10 text-blue-400 border-blue-500/30" },
  "Procure Inventory": { border: "border-violet-500/30", icon: "🛒", badge: "bg-violet-500/10 text-violet-400 border-violet-500/30" },
  "Emergency Replenishment": { border: "border-red-500/30", icon: "🚨", badge: "bg-red-500/10 text-red-400 border-red-500/30" },
  "Delay Shipment": { border: "border-amber-500/30", icon: "⏸", badge: "bg-amber-500/10 text-amber-400 border-amber-500/30" },
};
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={onGenerate}
          disabled={loading}
          className="px-4 py-2 text-sm rounded-lg bg-violet-600 hover:bg-violet-500 text-white transition disabled:opacity-50"
        >
            ...
            return (
              <div key={rec.id} className={`bg-gray-900 border rounded-xl overflow-hidden ${style.border}`}>
                ...
                <div className="px-5 py-4 space-y-3">
                  ...
                  {/* Deficit */}
                  {details.deficit !== undefined && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">Units Needed</span>
                      <span className="font-bold text-red-400">{Number(details.deficit).toLocaleString()} units</span>
                    </div>
                  )}
